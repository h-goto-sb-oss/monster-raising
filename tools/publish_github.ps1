...
# ============================================================
# Windows PowerShell 5.1 は、外部コマンド(gh/git/npm)が進捗などを
# stderr に書いただけで「エラー」と解釈する。"Stop" にすると
# 正常な実行でも途中で止まってしまうので Continue にして、
# 要所で $LASTEXITCODE を自分で確かめる。
...
if ($LASTEXITCODE -ne 0) { throw "gh-pages用の作業フォルダを作れませんでした" }
...
if ($pushed -ne 0) { throw "gh-pages ブランチのpushに失敗しました" }

Write-Host ""
Write-Host "== 4. GitHub Pages を有効化します ==" -ForegroundColor Cyan
# 初回は POST で作る。2回目以降は既にあるので POST が失敗する → PUT で更新する。
